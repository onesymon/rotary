    ...
    if (empty($passwordError)) {
        ...
        if ($age < 18) {
            ...
            if ($conn->query($updateQuery) === TRUE) {
                ...
                header("Location: /rotary/webpages/club-members/manage-members/manage_members.php");
                exit();
            } else {
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card card-primary">
                            <div class="card-header">
                                <h3 class="card-title">Edit Member Profile</h3>
                            </div>
                           ...
                           <form id="editProfileForm" method="post" action="" enctype="multipart/form-data">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="mb-2 d-flex justify-content-between align-items-center">
                                        ...
                                        <button type="button" class="btn btn-sm btn-outline-secondary" data-toggle="collapse" data-target="#passwordCollapse">
                                            Toggle
                                        </button>
                                    </div>
                                    ...
                                    <?php if ($_SESSION['role'] === '1' || $_SESSION['role'] === '100'): ?>
                                        <hr>
                                        <h5 class="text-primary"><i class="fas fa-user-tag"></i> Member Role</h5>
                                        <div class="form-group">
                                            <label for="role">Role</label>
                                            <select class="form-control" id="role" name="role" required>
                                                <option value="" disabled>Select Role</option>
                                                <?php
                                                $roleQuery = $conn->query("SELECT id, position_name FROM club_position WHERE id != 100 ORDER BY id ASC");
                                                while ($role = $roleQuery->fetch_assoc()):
                                                    $selected = ($memberDetails['role'] == $role['id']) ? 'selected' : '';
                                                ?>
                                                    <option value="<?php echo $role['id']; ?>" <?php echo $selected; ?>>
                                                        <?php echo htmlspecialchars($role['position_name']); ?>
                                                    </option>
                                                <?php endwhile; ?>
                                            </select>
                                        </div>
                                ...
                                <div class="card-footer text-right">
                                    ...
                                    <a href="/rotary/webpages/club-members/manage-members/manage_members.php" class="btn btn-secondary"><i class="fas fa-times"></i> Cancel</a>
                                </div>
                            </form>
                        </div>
    ...
    $(document).ready(function () {
        ...
        form.on('submit', function (e) {
            ...
            if (!this.checkValidity()) {
                return; // Let the browser show the validation UI
            }
        });

            if (!form.data('confirmed')) {
                e.preventDefault(); // Block the actual submission
                $('#confirmModal').modal('show'); // Show your modal
            }
        });
    ...
    $(document).ready(function () {
        $('form').on('submit', function (e) {
            ...
            if (!isAtLeast18YearsOld(dobValue)) {
                e.preventDefault();
                showClientValidationError("Member must be at least 18 years old.");
                return;
            }
